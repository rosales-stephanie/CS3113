...
//
    ...
    //horizontal wall by the entrance 9
    int depth = 0;
    for (int i = 0; i < 3; i++) {
        for (int j = 1; j < 4; j++){
    ...
    // extra block to make it look prettier 3
    for (int i = 0; i < 1; i++) {
        for (int j = 1; j < 4; j++){
            state.objects[count].position = glm::vec3(i,j,-5);
            state.objects[count].textureID = crateTextureID;
    ...
    // total: 73
    //horizontal to the left above 9
    depth = 0;
    for (int i = 0; i < 3; i++) {
        for (int j = 1; j < 4; j++){
    ...
    //horizontal wall to the right 9
    depth = -5;
    for (int i = 0; i < 3; i++) {
        for (int j = 1; j < 4; j++){
